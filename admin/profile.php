...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $action = $_POST['action'] ?? '';
    if ($action === 'update_profile' || $action === 'update_profile_photo') {
        ...
        if (($_POST['photo_action'] ?? 'upload') === 'remove') {
            ...
            if ($action !== 'update_profile') $accountError = 'Please choose a profile photo to upload.';
        } else {
...
$adminInitial = strtoupper(substr(trim((string) ($account['first_name'] ?? '')), 0, 1)) ?: '?';
...
?>
<section class="page-intro"><h2>My Account</h2><p>Manage your administrator account details and security.</p></section>
...
<section class="panel admin-account-card admin-account-single">
    <div class="admin-account-section"><span class="admin-account-kicker">Your information</span><h3>Personal Details</h3><p>These details are connected to your administrator account.</p>
        <div class="admin-account-profile-photo"><form id="admin-profile-photo-form" class="admin-account-photo admin-profile-photo-form" method="POST" enctype="multipart/form-data"><input type="hidden" name="action" value="update_profile_photo" /><div class="admin-account-photo-preview"><?php if ($profileImage): ?><img src="<?php echo htmlspecialchars($profileImage); ?>" alt="Profile photo preview" /><?php else: ?><span><?php echo htmlspecialchars($adminInitial); ?></span><?php endif; ?></div><div class="admin-account-photo-actions"><h3>Profile</h3><p>Add a photo so your account is easier to recognize.</p><button class="admin-button secondary" type="button" data-admin-photo-trigger><i class="fa-solid fa-camera"></i> Choose Photo</button><input id="admin-profile-photo" class="sr-only" type="file" name="profile_photo" accept="image/jpeg,image/png,image/webp" /><small>JPG, PNG, or WEBP up to 5 MB.</small><button class="admin-account-remove-button" type="submit" form="admin-remove-photo-form"><i class="fa-solid fa-trash-can"></i> Remove Photo</button></div></form><?php if ($profileImage): ?><form id="admin-remove-photo-form" method="POST" class="admin-account-remove-form"><input type="hidden" name="action" value="update_profile_photo" /><input type="hidden" name="photo_action" value="remove" /></form><?php endif; ?></div>
        <div class="form-grid two admin-account-details"><div class="form-field"><label for="admin-first-name">First Name</label><input id="admin-first-name" type="text" value="<?php echo htmlspecialchars($account['first_name'] ?? ''); ?>" readonly /></div><div class="form-field"><label for="admin-last-name">Last Name</label><input id="admin-last-name" type="text" value="<?php echo htmlspecialchars($account['last_name'] ?? ''); ?>" readonly /></div><div class="form-field"><label for="admin-phone">Phone Number</label><input id="admin-phone" type="text" value="<?php echo htmlspecialchars($account['phone'] ?? ''); ?>" readonly /></div><div class="form-field"><label for="admin-email">Email</label><input id="admin-email" type="email" value="<?php echo htmlspecialchars($account['email'] ?? ''); ?>" readonly /></div></div>
        <button class="admin-button primary admin-profile-save" type="submit" form="admin-profile-photo-form">Save Profile</button>
    </div>
    <form class="admin-form admin-account-form admin-account-section staff-password-section" method="POST"><input type="hidden" name="action" value="change_password" /><h3>Password &amp; Security</h3><p>Keep your administrator account password secure.</p><div class="form-grid two"><div class="form-field"><label for="current-password">Current Password</label><input id="current-password" type="password" name="current_password" required /></div><div class="form-field"><label for="new-password">New Password</label><input id="new-password" type="password" name="new_password" minlength="8" required /><div class="staff-password-strength" data-admin-password-strength><div><span>Password strength</span><strong data-admin-strength-label>Enter a password</strong></div><div class="staff-password-strength-track"><span data-admin-strength-bar></span></div><small data-admin-strength-help>Use at least 8 characters with uppercase, lowercase, number, and symbol.</small></div></div><div class="form-field"><label for="confirm-password">Confirm Password</label><input id="confirm-password" type="password" name="confirm_password" minlength="8" required /></div></div><div class="modal-actions"><button class="admin-button primary" type="submit"><i class="fa-solid fa-lock"></i> Change Password</button></div></form>
</section>

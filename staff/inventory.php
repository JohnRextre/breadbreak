...
$categories = $pdo->query('SELECT id, name FROM menu_categories ORDER BY id ASC')->fetchAll();
$categoryNameMap = [];
...
function saveInventoryPhoto(int $itemId): ?array {
    if (empty($_FILES['photo']['name']) || $_FILES['photo']['error'] === UPLOAD_ERR_NO_FILE) return null;
    if ($_FILES['photo']['error'] !== UPLOAD_ERR_OK || !is_uploaded_file($_FILES['photo']['tmp_name'])) throw new RuntimeException('Invalid photo upload.');
    ...
    if ($contents === false) throw new RuntimeException('Unable to read the item photo.');
...
?>
<section class="page-intro users-page-intro inventory-page-intro"><div><h2>Inventory</h2><p>Manage BreadBreak products, menu categories, and stock.</p></div><button class="admin-button primary" type="button" data-open-staff-modal="add-item-modal">+ Add Item</button></section>
